...
## 更新摘要
**所做更改**
- 新增环境变量支持章节，详细介绍 STORAGE_MOUNT 和 CUSTOM_STORAGE_MOUNT 环境变量
- 更新容器化部署配置指南，包含环境变量配置方法
- 增加备份策略章节，涵盖自定义存储挂载点的备份注意事项
- 扩展故障排除指南，增加环境变量相关的故障诊断方法
- 更新性能优化章节，包含自定义挂载点的性能考量
...
- 环境变量支持：STORAGE_MOUNT 和 CUSTOM_STORAGE_MOUNT 环境变量配置与使用
- 容器化部署：Docker/Kubernetes 环境下的存储配置最佳实践
- 备份策略：自定义存储挂载点的数据备份与恢复
...
subgraph "前端层"
W1["web/src/views/settings/index.vue"]
end
H1 --> S2
H1 --> S3
H1 --> S4
...
- [server/handler/settings.go:231-249](file://server/handler/settings.go#L231-L249)
...
- [web/src/views/settings/index.vue:1468-1485](file://web/src/views/settings/index.vue#L1468-L1485)
...
- 配额服务
  ...
  - 环境变量支持：STORAGE_MOUNT 和 CUSTOM_STORAGE_MOUNT 环境变量配置
- 模板服务
...
- 环境变量支持
  - STORAGE_MOUNT：用户存储挂载点路径，默认 "/var/lib/kvm-user-storage"
  - CUSTOM_STORAGE_MOUNT：自定义存储挂载点路径，支持用户自定义位置
  - 通过环境变量实现灵活的存储位置配置
- 关键流程图（用户配额设置）
...
### STORAGE_MOUNT 环境变量
STORAGE_MOUNT 环境变量用于指定用户存储挂载点的位置，默认值为 "/var/lib/kvm-user-storage"。

- 默认行为
  - 当未设置 STORAGE_MOUNT 时，系统使用默认挂载点 "/var/lib/kvm-user-storage"
  - 该路径在安装脚本和配额服务中均有硬编码引用
- 配置方法
  - Linux 系统：在 `/etc/environment` 或 systemd 环境文件中设置
  - Docker：使用 `-e STORAGE_MOUNT=/custom/path` 参数
  - Kubernetes：在 Deployment 的环境变量中配置

### CUSTOM_STORAGE_MOUNT 环境变量
CUSTOM_STORAGE_MOUNT 环境变量提供更灵活的自定义存储挂载点配置选项。
...
- 功能特性
  ...
  - 适用于特殊存储架构或容器化部署场景
- 使用场景
  - 容器持久化存储：将挂载点指向 Docker 卷或 Kubernetes PVC
  - 分布式存储：指向 NFS、CephFS 或其他网络存储
  - 多租户隔离：为不同租户配置独立的存储挂载点
...
```bash
docker run -d \
  --name open-vm-console \
  -e STORAGE_MOUNT=/custom/storage \
  -e CUSTOM_STORAGE_MOUNT=/data/openvm/storage \
  -v storage-volume:/custom/storage \
  -p 8080:8080 \
  open-vm-console:latest
```
...
spec:
  ...
  template:
    spec:
      containers:
      - name: open-vm-console
        image: open-vm-console:latest
        env:
        - name: STORAGE_MOUNT
          value: "/custom/storage"
        - name: CUSTOM_STORAGE_MOUNT
          value: "/data/openvm/storage"
        volumeMounts:
        - name: storage-volume
...
1. **启动日志检查**
   ```bash
   docker logs open-vm-console | grep "STORAGE_MOUNT\|CUSTOM_STORAGE_MOUNT"
   ```

2. **运行时配置查询**
   ```bash
   curl http://localhost:8080/api/settings | grep -E "(storage|mount)"
   ```

3. **文件系统验证**
   ```bash
   ls -la /custom/storage
   df -h /custom/storage
   ```

**章节来源**
- [server/service/storage/quota/quota.go:23-54](file://server/service/storage/quota/quota.go#L23-L54)
- [install.sh:27-348](file://install.sh#L27-L348)
- [install.sh:425-450](file://install.sh#L425-L450)
...
- 环境变量与配置管理
  - STORAGE_MOUNT 和 CUSTOM_STORAGE_MOUNT 环境变量通过配置系统统一管理
  - 支持运行时动态配置更新
...
S --> Env["环境变量<br/>STORAGE_MOUNT<br/>CUSTOM_STORAGE_MOUNT"]
Env --> Cfg["配置系统"]
```
...
- 环境变量性能影响
  - 自定义存储挂载点可能影响 I/O 性能，需根据存储介质特性选择合适的挂载路径
  - 网络存储（NFS/CephFS）可能引入延迟，需进行性能基准测试
...
- 环境变量配置问题
  - 现象：自定义存储挂载点不生效或权限错误
  - 处理：检查环境变量是否正确设置、挂载点权限是否正确、SELinux/AppArmor 是否阻止访问
  - 参考：[server/service/storage/quota/quota.go:23-54](file://server/service/storage/quota/quota.go#L23-L54)
  - 参考：[install.sh:425-450](file://install.sh#L425-L450)
...
- 环境变量支持，提供灵活的存储位置配置能力
- 容器化部署兼容性，支持 Docker 和 Kubernetes 环境
- 完善的备份策略和故障恢复机制

建议在生产环境结合监控与告警完善容量预警与配额审计，同时根据实际部署环境选择合适的存储挂载点配置。
...
- 关键术语
  ...
  - 环境变量：STORAGE_MOUNT、CUSTOM_STORAGE_MOUNT
- 常用命令
  ...
  - env_get/env_set：环境变量管理工具
- 配置文件位置
  - /etc/fstab：挂载配置文件
  - /etc/projects：项目配额映射
  - /etc/projid：项目ID映射
  - /etc/environment：系统环境变量
  - /var/lib/kvm-user-storage：默认存储挂载点