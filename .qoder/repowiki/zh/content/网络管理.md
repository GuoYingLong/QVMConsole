...
**更新** 本次更新重点关注数据库层面的VPCSwitch模型CIDR字段映射修复，确保GORM ORM正确识别和处理CIDR字段，避免因字段命名映射问题导致的数据不一致和查询异常。
...
**阶段0：修复现有问题**
- 检测是否存在c_id_r旧列和cidr新列
- 将c_id_r中的数据迁移到cidr列（仅更新cidr为空的记录）
- 创建唯一索引idx_vpc_switches_cidr
- 删除旧的无效索引idx_vpc_switches_c_id_r
...
**更新** 通过VPCSwitch模型CIDR字段映射修复和数据库迁移策略，进一步增强了系统的数据一致性和可靠性，为网络管理提供了更加稳固的基础设施支撑。
