...
func startVM(name string, fixOnReboot bool) error {
	if err := HookEnsureVMNotMigrating(name, "开机"); err != nil {
		return err
	}
	...
	if !started {
		...
		if result.Error != nil {
			...
			if strings.Contains(result.Stderr, "Permission denied") {
				snapshot.FixSnapshotDiskPermissions(name)
...
func ShutdownVM(name string) error {
	if err := HookEnsureVMNotMigrating(name, "关机"); err != nil {
		return err
	}
...
func DestroyVM(name string) error {
	if err := HookEnsureVMNotMigrating(name, "强制断电"); err != nil {
		return err
	}
...
func RebootVM(name string) error {
	if err := HookEnsureVMNotMigrating(name, "重启"); err != nil {
		return err
	}
...
func ResetVM(name string) error {
	if err := HookEnsureVMNotMigrating(name, "重置"); err != nil {
		return err
	}
