...
func EditVMConfig(name string, vcpu, maxVCPU, memoryMB int) error {
	if err := HookEnsureVMNotMigrating(name, "编辑配置"); err != nil {
		return err
	}
	// 检查虚拟机状态
...
func SetVMAutostart(name string, autostart bool) error {
	if err := HookEnsureVMNotMigrating(name, "设置开机自启"); err != nil {
		return err
	}
...
func SetVMBootOrder(name string, bootOrder []string) error {
	if err := HookEnsureVMNotMigrating(name, "设置启动顺序"); err != nil {
		return err
	}
	// 先尝试 virt-xml 方式
