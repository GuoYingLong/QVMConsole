...
func CheckAndApplyTrafficLimit(username string) {
	...
	// 两个方向都不限制，跳过
	if user.MaxTrafficDown <= 0 && user.MaxTrafficUp <= 0 {
		return
	}
	...
	// 检查下行是否超限
	if user.MaxTrafficDown > 0 && float64(effectiveDown) >= user.MaxTrafficDown*1024*1024*1024 {
	...
	// 检查上行是否超限
	if user.MaxTrafficUp > 0 && float64(effectiveUp) >= user.MaxTrafficUp*1024*1024*1024 {
...
func applyTrafficPenalty(username string, downLimited, upLimited bool) {
	...
	for _, vmName := range vms {
		...
		// 获取当前带宽设置
		currentBW, err := GetVMBandwidth(vmName)
		if err != nil {
			// 可能没有网卡，跳过
			continue
		}
		...
		// burst 最小值
		downBurst := MbpsToKBps(downAvg) * 30
		upBurst := MbpsToKBps(upAvg) * 30
...
func CheckAllUsersTrafficQuota() {
	...
	for _, user := range users {
		...
		CheckAndApplyTrafficLimit(user.Username)
	}
	CheckAllVPCSwitchTrafficQuota()
	CheckAllLightweightVMTrafficQuota()
}
