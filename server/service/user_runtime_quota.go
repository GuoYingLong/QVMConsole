...
func getRuntimeActiveVMSetFromHost() (map[string]struct{}, error) {
	...
	for _, args := range [][]string{
		...
		if result.Error != nil {
			...
			if IsMaintenanceModeEnabled() && (isLibvirtUnavailableText(message) || IsLibvirtUnavailableError(result.Error)) {
...
func sendRuntimeQuotaWarningEmail(user *model.User, snapshot UserRuntimeQuotaSnapshot) error {
	...
	if err := SendEmail(user.Email, "总运行时长配额即将耗尽", body); err != nil {
		return err
	}
